...
2. **Brand portfolio with watermarked print docs** (highest-ROI NEW surface; sells the $1,500 brand-only buyer). A "Brand Work" gallery: business-card mockups, letterhead flatlays, brand-book spreads, packaging — each with a subtle `scs.ai` watermark, lightbox on click. **Source packs are bundled in `_HANDOFF/brand-packs/`** (so this works on the laptop with no file transfer): `scs-brand-book/` (32-page SCS brand book PNGs), `scs/` + `coliseum/` (brand guideline PDFs), `Branding Package Design.pdf`. PDFs need rendering to images; the PNG brand-book pages are display-ready. **← recommended immediate start; needs nothing from Marcus.**
